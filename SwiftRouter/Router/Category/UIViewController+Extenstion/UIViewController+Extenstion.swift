...
extension UIViewController {
    ...
    func createRightBarBtnItem(btns:[UIButton]){
        var barItems = [UIBarButtonItem]();
        for btn in btns{
            btn.frame = CGRect(x: 0, y: 0, width: 30, height: 30)
            barItems.append(UIBarButtonItem(customView: btn))
        }
        self.addFixedSpace(barItems: barItems, direction: .right);
    }
    
    
    
    func createRightBarBtnItem(title: String, method: Selector, titleColor: UIColor = UIColor.white) {
        let rightBtn = self.createBarBtnItem(title: title, method: method, titleColor: titleColor)
        self.addFixedSpace(with: rightBtn, direction: .right)
    }
    
    func createRightCustomViewBarItem(customView:UIView){
    ...
    func createLeftBarBtnItem(title: String, method: Selector?, titleColor: UIColor = UIColor.white) {
        let leftBtn = self.createBarBtnItem(title: title, method: method != nil ? method!:#selector(cancel), titleColor: titleColor)
        self.addFixedSpace(with: leftBtn, direction: .left)
    }
    func createLeftBarBtnItem(Image: UIImage, method: Selector?, titleColor: UIColor = UIColor.white) {
        
        let leftBtn = self.createBarBtnItem(image: Image, method: method != nil ? method!:#selector(cancel));
        self.addFixedSpace(with: leftBtn, direction: .left)
    }
    // MARK: 私有
    ...
    fileprivate func createBarBtnItem(image: UIImage, method: Selector, titleColor: UIColor = UIColor.white) -> UIBarButtonItem {
        let originBtn = UIButton(frame: CGRect.zero)
        //        originBtn.titleLabel?.font = UIFont.systemFont(ofSize: 15.0)
        //        originBtn.setTitle(title, for: .normal)
        //        originBtn.setTitleColor(titleColor, for: .normal)
        //        originBtn.setTitleColor(titleColor.withAlphaComponent(0.5), for: .highlighted)
        originBtn.setImage(image, for: .normal);
        originBtn.sizeToFit()
        originBtn.addTarget(self, action: method, for: .touchUpInside)
        return UIBarButtonItem(customView: originBtn)
    }
    ...
    fileprivate func addFixedSpace(barItems: [UIBarButtonItem], direction: BarBtnItemDirection) {
        let negativeSpacer = UIBarButtonItem(barButtonSystemItem: .fixedSpace, target: nil, action: nil)
        negativeSpacer.width = -11
        switch direction {
        case .left:
